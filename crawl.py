                        ...
                        if len(info[key]) == 0 and not key in null:
                            ...
                            for url in info[key]:
                                if not url in data[key]:
                                    data[key].append(url)
    with open(os.path.join(os.getcwd(), "FinalGoogle.json")) as data_file:
        utl = json.load(data_file)
        for row in null:
            if row in info:
                null.remove(row)
...
def getgoogle():
    ...
    for row in a:
        urls = []
        print('\x1b[0;30;44m' + "Done ! " + '\x1b[0m' + "\n")
        print("")
        try:
...
def cartesian():
    ...
    a = ['Anscombe quartet', 'donut', 'upset', 'pie scatter', 'ring', 'Sparkline', 'Chernoff’s faces',
         'Minard’s map',
         ...
         'domain specific', 'word tree', 'job voyager', 'Kagi Chart', 'Election map', 'spider', 'fan',
         'map of the market']
